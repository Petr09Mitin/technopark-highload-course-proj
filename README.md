...
<ol>
    ...
    <li>MongoDB fails (Profile service) - Memcached is read from</li>
    <li>Механизм preview для медиафайлов - в случае, если основные обработчики медиа не успели обработать файлы, пользователю отдается preview, 
    обработка и сохранение которого должна происходить за меньшее время</li>
    <li>Применение паттерна Orchestrated Saga для флоу регистрации</li>
    <li>Применение event sourcing для асинхронной репликации данных в сервисах. 
    Например, при регистрации данные профиля диспатчатся в виде события в Kafka и асинхронно записываются в Profile Service</li>
</ol>
