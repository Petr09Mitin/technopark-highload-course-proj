...
<table>
    ...
    <tbody>
        <tr>
            <td>Зарегистрироваться по номеру телефона</td>
            <td>
            API Gateway -> ProfileService -> Save insensitive data (UserStorage, UserAvatar, PhoneSearch) -> dispatch PhoneSearch update
            Результаты - в API Gateway -> Auth Service -> сохранить пароль, выдать сессию -> вернуть 
            </td>
        </tr>
        <tr>
            <td>Войти по номеру телефона</td>
            <td>
                API Gateway -> AuthService -> Search for userId, verify password, sign session -> API Gateway
            </td>
        </tr>
        <tr>
            <td>Отправить сообщение (возможно с вложением)</td>
            <td>
                API Gateway -> ChatCallService -> Save attachments, save message, dispatch message event -> MessageDispatcher reads event -> sends through XMPP server 
            </td>
        </tr>
        <tr>
            <td>Пометить сообщение как RECEIVED/READ</td>
            <td>
                API Gateway -> ChatCallService -> dispatch event, if event is RECEIVED, delete message -> MessageDispatcher reads event -> sends through XMPP server
            </td>
        </tr>
        <tr>
            <td>Найти пользователя по номеру телефона</td>
            <td>
                ProfileService -> get userID from ElasticSearch -> Retrieve data from UserStorage by ID -> Return
            </td>
        </tr>
        <tr>
            <td>Сделать аудиозвонок</td>
            <td>
